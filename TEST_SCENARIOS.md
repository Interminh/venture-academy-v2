# Venture Academy Tutors: Test Scenarios
...
| `signUp` | Boundary | Display name / email with leading/trailing whitespace gets trimmed | Not tested |
| `signUp` | Invalid | Missing email/password/displayName → generic "fill in every field" error | ✅ Pass (blocked client-side) |
| `signUp` | Invalid | Malformed email (no @, no domain), Supabase-side validation only, confirm behavior | Not tested |
| `signUp` | Invalid | Tutor code with wrong case / extra whitespace / SQL-special chars | Not tested |
| `signUp` | Security | Tutor code brute-force: repeated invalid-code submissions (no rate limit exists per README) | Not tested. Known accepted gap, not stress-tested |
...
| `signUp` | Network/failure | `is_email_registered` RPC missing/erroring (migration 0007 not applied), confirm fallback to normal signUp path actually works | Not tested. N/A on prod (migration applied) |
| `signUp` | Concurrency | Two signups with the same email submitted simultaneously | Not tested |
...
| `signIn` | Invalid | Unconfirmed email → distinct "confirm your email" message | Not tested. N/A on prod (auto-confirm) |
...
| `signIn` | Boundary | Empty email or password field | Not tested. Blocked client-side (required attr), not separately probed |
| `signOut` | Auth | Sign out while already logged out (no session) | Not tested |
...
| `requestPasswordReset` | Boundary | Empty email input | Not tested |
| `requestPasswordReset` | Time-based | Reset link used after expiry | Not tested |
| `requestPasswordReset` | Concurrency | Multiple reset requests for the same email in quick succession (no app-level throttle) | Not tested. Known accepted gap |
| `resetPassword` | Auth | Submit with no active recovery session → "link has expired" | Not tested |
| `resetPassword` | Boundary | Password exactly 8 chars | Not tested |
| `resetPassword` | Security | Reset link reused twice (session consumed after first use?) | Not tested |
| `/auth/callback` | Invalid | Missing `code` param → redirect to `/login` | Not tested |
| `/auth/callback` | Invalid | Invalid/expired/already-used `code` → redirect to `/login` | Not tested |
| `/auth/callback` | Security | Tampered `next` param, open-redirect check (does it allow `next=https://evil.com`?) | ✅ **Pass (fixed)**, found via code review, not live-tested with a real reset code, see finding below |
| `updateSession` (proxy) | Auth | Logged-out user hits `/dashboard/*` → redirected to `/login?next=...` | ✅ Pass |
| `updateSession` (proxy) | Auth | Tutor hits `/dashboard/admin/*` directly by URL → redirected to own role home | Not tested |
| `updateSession` (proxy) | Auth | Parent hits `/dashboard/tutor/*` directly by URL | Not tested |
| `updateSession` (proxy) | State transition | Admin demotes a logged-in admin's own session mid-session (another admin does it), does the demoted user's *next* request get redirected out of `/dashboard/admin`? | Not tested |
| `updateSession` (proxy) | Network/failure | Supabase Auth endpoint unreachable/paused project, confirm fail-open behavior doesn't crash and doesn't wrongly grant access | Not tested. Can't safely simulate against prod |
| `updateSession` (proxy) | Network/failure | Profile row missing/query fails for a logged-in user, confirm fail-open path | Not tested |
| `createTutee` | Valid | Full valid submission: name, grade 0–8, ≥1 subject, ≥1 slot | ✅ Pass |
| `createTutee` | Boundary | Grade 0 (Kindergarten) and grade 8 (max) both accepted | Not tested |
...
| `createTutee` | Invalid | Missing firstName, non-numeric grade, zero subjects, zero slots, each produces its own error | Not tested |
| `createTutee` | Invalid | `maxWeeklySessions` = non-numeric string | Not tested |
| `createTutee` | Security | XSS payload in `firstName`/`notes` (e.g. `<script>`), confirm rendered escaped everywhere it's displayed (parent, tutor, admin views) | Not tested. (XSS tested on subjects/force-cancel reason instead, see below, same escaping mechanism, not separately re-tested here) |
| `createTutee` | State transition | Tutee insert succeeds but subsequent `tutee_subjects` or `availability_slots` insert fails, is the tutee left in a half-created state (no transaction wrapping these three inserts)? | Not tested |
| `updateTutee` | Valid | Add a subject, remove a subject, add/remove slots in one submit | Not tested |
| `updateTutee` | State transition | Unchecking a slot with a *pending* claim → claim auto-cancelled, slot soft-deleted | Not tested |
| `updateTutee` | State transition | Unchecking a slot with an *approved* claim → same auto-cancel path, confirm tutor sees it disappear/cancel | Not tested |
| `updateTutee` | Concurrency | Parent edits availability at the same moment a tutor claims the slot being removed | Not tested |
| `updateTutee` | Boundary | Re-adding a previously removed-then-readded slot at the same day/time (soft-delete + new active row), confirm old cancelled claim history isn't resurrected or duplicated | Not tested |
| `deleteTutee` | State transition | Deleting a tutee with live (pending/approved) claims → claims auto-cancelled, not orphaned | Not tested |
| `deleteTutee` | State transition | Deleting an already-deleted (is_active=false) tutee again | Not tested |
...
| `dismissDeletedTutee` | Auth | Non-admin calls this action directly (bypassing UI) | Not tested |
| `dismissDeletedTutee` | Invalid | Dismissing a tutee that is still active (not soft-deleted) | Not tested |
| `submitClaim` | Valid | Tutor claims an open slot for a subject the student needs | ✅ Pass |
| `submitClaim` | Invalid | slotId/subjectId missing or empty | Not tested |
| `submitClaim` | Invalid | subjectId not in the student's needed-subjects list | Not tested |
| `submitClaim` | Invalid | slotId for a slot that's been soft-deleted (`is_active=false`) | Not tested |
...
| `submitClaim` | Auth | Logged-out user submits claim (via direct form POST, bypassing UI) | Not tested |
...
| `submitClaim` | Security | Claim submitted for a slot belonging to a tutee the tutor shouldn't be able to see (enumerated slotId) | Not tested |
| `cancelOwnClaim` | Auth | Tutor tries to cancel another tutor's approved claim | Not tested |
| `cancelOwnClaim` | State transition | Cancel a claim that's already been force-cancelled/rejected by an admin moments earlier | Not tested |
| `cancelOwnClaim` | Invalid | claimId for a claim not in 'approved' status (e.g. still pending) | Not tested |
| `dismissClaim` | Invalid | Dismiss a claim that's still pending/approved (only cancelled/rejected allowed per RLS) | Not tested |
...
| `approveClaim` | State transition | Approve a claim that a tutor cancelled a second ago (race between admin approve and tutor cancel) | Not tested |
| `approveClaim` | State transition | Approve the same claim twice (double-click / double form submit) | Not tested |
| `approveClaim` | Network/failure | Resend API down/misconfigured mid-approval, confirm approval still commits (per code comment) and error is only logged, not surfaced as a failed approval | Not tested |
| `approveClaim` | Auth | Non-admin calls approveClaim directly | Not tested |
| `rejectClaim` | State transition | Reject a claim that was just approved by a different admin (double action race) | Not tested |
| `forceCancelClaim` | Valid | Force-cancel an approved claim with and without a reason string | ✅ Pass |
| `forceCancelClaim` | Boundary | Very long `reason` string | Not tested |
| `forceCancelClaim` | Security | XSS payload in `reason` field, rendered in admin ledger | ✅ Pass, escaped, not executed |
| `dismissLedgerClaim` | Invalid | Attempt to dismiss a claim still 'pending' or 'approved' (should be blocked, only cancelled/rejected) | Not tested |
...
| `renameSubject` | State transition | Rename a subject that's referenced by existing tutee_subjects/claims, confirm historical claims still display the new name (no snapshot) | Not tested. No rename UI currently exposed in the admin panel (action exists in code, unreachable from the UI), flagged, not a bug |
...
| `createTutorCode` | Security | Code guessing, is the code space large/random enough to resist enumeration given no rate limit? | Not tested. Codes are admin-chosen strings, not generated, enumeration risk depends entirely on how obvious the club's codes are, not a code-level issue |
...
| `updateUserRole` | Invalid | role value outside admin/tutor/parent (tampered form field) | Not tested |
| `updateUserRole` | State transition | Demote a tutor who has pending/approved claims, what happens to those claims? (not addressed in code, ask) | Not tested. Still an open question, see Phase 2 notes |
...
| `logHours` | Boundary | hours = 0 (rejected, must be > 0), hours = 24 (accepted, max), hours = 24.01 or >24 (rejected) | Not tested |
| `logHours` | Invalid | Non-numeric hours, missing sessionDate/studentLabel/description | Not tested |
| `logHours` | Boundary | sessionDate far in the future or far in the past, any bound? | Not tested |
| `logHours` | Security | XSS in studentLabel/description shown in admin stats/hours table | Not tested |
| `deleteHours` | Auth | Tutor deletes another tutor's hours entry (RLS should block) | Not tested |
...
| `unsubscribe/resubscribe` | Concurrency | Rapid toggle (double-click both buttons) | Not tested |
| `sendSessionBookedNotifications` | State transition | Parent or tutor has `notifications_enabled=false` → confirm the *other* party still gets their email independently | Not tested |
| `sendSessionBookedNotifications` | Network/failure | Resend rejects one recipient (invalid address), does it affect sending to the other recipient? | Not tested |
| `RealtimeRefresh` | Network/failure | Websocket/channel connection drops, does the page silently stop live-updating, or reconnect? | Not tested |
...
| Admin stats page | Boundary | Zero tutors/tutees/hours logged, confirm no divide-by-zero or NaN display | Not tested. N/A on prod (real data already present) |
| Admin stats page | Time-based | `tutor_hours` sort by `session_date` with entries logged for a future date or duplicate dates | Not tested |
...
| Intake/edit forms | Time-based | `START_TIMES` are fixed local strings (4:00–8:30pm) with no timezone stored, confirm no DST-boundary or timezone-shift bug when club moves across DST change (Nov/Mar) | Not tested |
| Middleware/proxy | Boundary | Matcher excludes `_next/static`, `_next/image`, `favicon.ico`, image extensions, confirm no route is unintentionally excluded/included (e.g. a `.svg` page route) | Not tested |
| Whole app | Network/failure | Supabase project paused (free-tier auto-pause after ~1 week idle), confirm `DatabaseUnavailable`/error boundaries show a clean message everywhere, not a stack trace | Not tested. Can't safely simulate against prod |
| Whole app | Security | CSRF: Server Actions rely on Next.js's built-in Origin check, confirm a cross-origin form POST to an action endpoint is rejected | Not tested |
| Marketing page | Valid | All static page links/nav render, contact us link works, no broken images | Not tested |
...
fixing the automated suite's one real finding (it initially failed, by
design, that's what caught it). A second finding, an open redirect, turned
up in a manual code review pass afterward and is fixed too, but isn't
...
### 🟢 Fixed: open redirect in the password-reset callback

`app/auth/callback/route.ts` built its redirect target as `${origin}${next}`,
with `next` read straight from the query string. A value like
`@evil.com/path` turns that into a URL a browser will treat as pointing at
`evil.com`: the `@` makes everything before it userinfo, not part of the
host. The app itself only ever sends `next=/reset-password`, so this is
only reachable if someone hand-crafts a callback link with both a valid
password-reset code and a malicious `next` value, narrow, but real.

Fixed by validating `next` is a plain same-origin path (starts with a
single `/`, no `@`) before using it, falling back to `/reset-password`
otherwise. Found via code review while doing a final pass before
considering the app launch-ready, not caught by the automated suite, so
it's not a regression test yet.
...
Rows marked "not tested" above weren't exercised this pass, mostly boundary/invalid
